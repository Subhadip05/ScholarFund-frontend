  ...
  template: `
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex items-center space-x-3">
          <div class="w-10 h-10 rounded-lg bg-blue-50 border border-blue-200 flex items-center justify-center text-blue-700">
          ...
          <div>
            ...
            <div class="text-xl font-black text-slate-900 font-mono mt-0.5">{{ institutes().length }}</div>
          </div>
        </div>
        ...
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex items-center space-x-3">
          ...
          <div>
            ...
            <div class="text-xl font-black text-amber-800 font-mono mt-0.5">{{ pendingCount() }}</div>
          </div>
        </div>

        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex items-center space-x-3">
          <div class="w-10 h-10 rounded-lg bg-emerald-50 border border-emerald-200 flex items-center justify-center text-emerald-700">
            <i class="pi pi-check-circle text-base leading-none"></i>
          </div>
          <div>
            <div class="text-xs text-slate-500 font-medium">Govt Verified & Active</div>
            <div class="text-xl font-black text-emerald-800 font-mono mt-0.5">{{ verifiedCount() }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs space-y-3">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div class="flex space-x-1 border border-slate-200 p-1 rounded-lg bg-slate-50 self-start text-xs">
            <button
              (click)="handleFilterTabChange('ALL')"
              [class]="filterTab() === 'ALL' ? 'bg-white text-slate-900 shadow-xs font-bold' : 'text-slate-600 hover:text-slate-900'"
            ...
            <button
              ...
              [class]="filterTab() === 'PENDING' ? 'bg-amber-100 text-amber-900 shadow-xs font-bold' : 'text-amber-800 hover:bg-amber-50'"
            ...
            <button
              ...
              [class]="filterTab() === 'VERIFIED' ? 'bg-emerald-100 text-emerald-900 shadow-xs font-bold' : 'text-emerald-800 hover:bg-emerald-50'"
              class="px-3 py-1.5 rounded-md font-semibold transition-colors flex items-center space-x-1.5 cursor-pointer"
            >
              ...
              @if (paginatedInstitutes().length === 0) {
                <tr>
                  <td colSpan="8" class="p-12 text-center text-slate-500">
                    <i class="pi pi-building-columns text-3xl text-slate-300 mx-auto mb-2 block"></i>
                ...
                @for (inst of paginatedInstitutes(); track inst.profileId) {
                  ...
                  >
                    ...
                    <td class="p-3.5 align-top">
                      ...
                      <span class="inline-block mt-1 px-1.5 py-0.5 rounded text-[9px] font-semibold bg-slate-100 text-slate-700 border border-slate-200 uppercase leading-none">
                    ...
                    <td class="p-3.5 align-top max-w-[180px]">
                      <div class="flex items-start text-xs text-slate-800 font-medium leading-snug">
                        <i class="pi pi-graduation-cap mr-1 text-blue-600 text-xs mt-0.5 shrink-0"></i>
                        <span class="truncate" [title]="inst.universityAffiliation">{{ inst.universityAffiliation }}</span>
                      </div>
                    </td>

                    <!-- Principal -->
                    <td class="p-3.5 align-top">
                      <div class="font-semibold text-slate-800 text-xs">{{ inst.principalName }}</div>
                    ...
                    <td class="p-3.5 align-top">
                      ...
                      <div class="text-[10px] text-slate-500 font-mono">{{ inst.officerPhoneNo }}</div>
                      <div class="text-[10px] text-blue-700 font-mono truncate max-w-[160px]" [title]="inst.email">
                        {{ inst.email }}
                      </div>
                    </td>
...
export class InstitutesViewComponent {
  ...
  readonly totalPages = computed(() => Math.ceil(this.filteredInstitutes().length / this.pageSize()) || 1);
